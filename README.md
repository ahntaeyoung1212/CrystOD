...
- `crystod-phonon --subgroup` for the isotropy subgroups reachable from the imaginary phonon modes: every imaginary level is labeled with its irrep and each order-parameter direction is resolved to the space group it condenses into (cubic SrTiO3: the R5- triplet gives I4/mcm, R-3c, Imma, ...), scanning every q point commensurate with the supercell when no `--qpoint` is given — the symmetry-lowering step of a structure search, in one command. (v0.3.6)
- **Python API**: every analysis is also a Python function, grouped into one module per command (`crystod.salc`, `crystod.group`, `crystod.phonon`, `crystod.bz`, `crystod.mag`, `crystod.md`, `crystod.mol`), so that a part of CrystOD can be used inside another program — e.g. `crystod.phonon.imaginary_mode_subgroups(phonon)` returns the subgroups of the imaginary modes of a live phonopy object as data. Importing is cheap: phonopy, spgrep, PySCF and matplotlib load only when a function that needs them is called. (v0.3.6)
...
This section describes the mathematical core of the package, not a command: there is no CLI mode for it, and it is used through the Python API only (see section 35). `crystod.operations.wigner_D_real(l, R)` returns the `(2l+1) x (2l+1)` matrix representing an O(3) operation `R` (a 3x3 Cartesian rotation, rotoinversion, or mirror matrix) on the real spherical harmonics of angular momentum `l` (s, p, d, f, ... orbitals for `l` = 0, 1, 2, 3, ...). Every orbital-symmetry feature of CrystOD is built on these matrices: the SALC/crystal-orbital characters (sections 2-3, 5), the ligand-field splitting (section 9), the polynomial basis classification (sections 10-11), and the spin-multipole bases of `crystod-mag` (section 28, through the axial-vector representation).
...
## 27. Subgroups from Imaginary Phonon Modes (`crystod-phonon --subgroup`; v0.3.6)

*Testsuite section 27*

An imaginary phonon means the structure is unstable against that mode, and the symmetry it can lower to is fixed by the irrep of the mode. `--subgroup` labels every imaginary mode and lists the isotropy subgroups of its irrep — the possible daughter phases — in one step, combining what `crystod-phonon --irreps` and `crystod-group --supergroup` do separately:

```bash
cd example/21_phonon_irrep/SrTiO3_Pm-3m
crystod-phonon --subgroup --dim "4 4 4" -c 221_PPOSCAR_SrTiO3
```
...
Without `--qpoint`, every q point commensurate with the supercell is scanned (64 of them for a 4x4x4 supercell) and the imaginary levels are reported most unstable first, so the unstable q points need not be known in advance. With `--qpoint R` (a label or three coordinates) only that q point is analyzed. `--threshold` sets the frequency below which a mode counts as imaginary (default `-0.1` THz), and `--yaml phonopy_params.yaml` may be used instead of `--dim`/`-c`.

Each listed direction is one order-parameter direction of the degenerate level. Freezing in a single eigenvector explores only one of them: the R-point triplet of cubic SrTiO3 gives `I4/mcm` along `(0,0,a)`, but the same triplet also reaches `R-3c` along `(a,a,a)` and `Imma` along `(0,a,a)` — the tilt systems a structure search has to try before it can claim a ground state. The corresponding distorted structures are generated with `crystod-phonon --modulation`, and the same enumeration is available programmatically as `crystod.phonon.imaginary_mode_subgroups` (see the Python API section).
...
## 35. Python API (v0.3.6)

*Testsuite section 35 — full guide: [doc/python-api.md](doc/python-api.md)*

Every CrystOD analysis is also available as a Python function, so that a part of CrystOD can be used inside another program. The API mirrors the command structure — one module per command, with the same vocabulary as the printed output:
...
Isotropy subgroups of an irrep, as data (the API form of `crystod-group --supergroup`):
...
Phonon modes labeled with ISO-IR irreps, and the subgroups their imaginary levels can condense into, straight from a live phonopy object — the symmetry-lowering step of a structure search:

```python
import phonopy
from crystod.phonon import label_phonon_modes, imaginary_mode_subgroups, scan_imaginary_modes

ph = phonopy.load(supercell_matrix=[4, 4, 4], primitive_matrix="auto",
                  unitcell_filename="221_PPOSCAR_SrTiO3", force_sets_filename="FORCE_SETS")

label_phonon_modes(ph, [0.5, 0.5, 0.5])[0]   # modes 1,2,3: -1.0867 THz  R5-

for result in imaginary_mode_subgroups(ph, [0.5, 0.5, 0.5]):
    for sub in result.subgroups:
        print(sub.label, "->", sub.symbol)    # R5-(0,0,a) -> I4/mcm, R5-(a,a,a) -> R-3c, ...

scan_imaginary_modes(ph)    # the same over every q point the supercell resolves
```

Build the phonopy object with `primitive_matrix="auto"`, so that a zone-boundary instability appears at its true q point instead of being folded onto the supercell Gamma point, where it cannot be labeled.

Importing is cheap: `import crystod` and the seven domain modules pull in nothing heavier than NumPy — phonopy, spgrep, spglib, PySCF and matplotlib load only when a function that needs them is actually called. The implementation modules (`crystod.phonon_irreps`, `crystod.isotropy_subgroup`, `crystod.operations`, ...) remain importable under their own names, so code written before the API modules existed keeps working unchanged.
...
- `crystod-phonon --subgroup --dim "nx ny nz" -c POSCAR [--qpoint QLABEL_OR_QX QY QZ] [--threshold -0.1] [--readfc] [--modulate [--amplitude A]]` (without `--qpoint`: every q point commensurate with the supercell is scanned; `--yaml phonopy_params.yaml` may replace `--dim`/`-c`; `--modulate`: also write the distorted structure of every order-parameter direction)
- `crystod-phonon --modulation -c POSCAR --qpoint qx qy qz [--mode MODE1 MODE2 ...] [--amplitude A1 A2 ...] [--dim "nx ny nz"] [--readfc] [--output FILE]` (unit cell + FORCE_SETS/FORCE_CONSTANTS, or `--yaml phonopy_params.yaml` instead of `-c`; without `--dim` the supercell is read from `phonopy_disp.yaml` or inferred from the force file; without `--mode`: mode table + star of q only; default output: `MPOSCAR_{q}_{mode}_{irrep}_{subgroup}`)
...
- `crystod-group --supergroup SG --irrep IR [--order-parameter 0 0 a]` (`--parent SG` is the same option, named after the value it takes)
