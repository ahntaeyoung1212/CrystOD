...
Sections:
   ...
   4. crystod-group --basis     polynomial basis classification
   5. crystod-group --product   point-group direct products
   6. crystod-phonon --irreps   phonon irrep labeling (phonopy data)
   7. crystod-phonon --vibration symmetry-only vibration bases
   8. crystod-phonon --modulation modulated structures (known space groups)
   9. crystod --star-of-k       star of k
  10. crystod-group --coset     coset decompositions
  11. crystod-group --generate-basis automatic polynomial bases
  12. crystod --visualize       SALC coefficients + 3D HTML viewer
  13. crystod-bz                Brillouin-zone plot (seekpath auto k-path)
  14. crystod-phonon --vector   phonon eigenvector VESTA export (phonopy data)
  15. crystod-group --decompose reducible-representation decomposition
  16. crystod-md --adp          ADPs from an MD XDATCAR trajectory
  17. crystod-phonon --fatband  element-projected phonon fatbands (phonopy data)
  18. crystod-bz --trans-mat    unit-cell + supercell Brillouin-zone plot
  19. crystod-phonon --lt       longitudinal/transverse-resolved phonon band
  20. crystod-group --ligand-field orbital splitting in a point-group field
  21. crystod-mag               symmetry-adapted spin bases (cluster multipoles / SAMM)
  22. crystod-bz                sectioned-command extras (identity/errors/removed flags)
  23. crystod-md                --adp / --summary extras
  24. crystod-mag               --format qe / --conventional extras
  25. crystod-phonon            six-mode extras
  26. crystod-group             seven-mode extras
  27. crystod main command      SALC without mode flag, --visualize, --star-of-k
"""
...
# ---------------------------------------------------------------- 22. crystod-bz
def test_22_bz_command() -> None:
    print("\n[22] crystod-bz (sectioned command: unit-cell / supercell BZ)")
...
# ---------------------------------------------------------------- 23. crystod-md
def test_23_md_command() -> None:
    print("\n[23] crystod-md (sectioned command: MD trajectory -> ADPs / summary)")
    ...
    if not os.path.isfile(source):
        ...
        return
    with tempfile.TemporaryDirectory() as tmp:
        # truncated trajectory (293 frames), as in section 16
        destination = os.path.join(tmp, "XDATCAR")
        with open(source) as fin, open(destination, "w") as fout:
            for line_number, line in enumerate(fin):
                if line_number >= 77352:
...
# ---------------------------------------------------------------- 24. crystod-mag
def test_24_mag_command() -> None:
    print("\n[24] crystod-mag (sectioned command: symmetry-adapted spin bases)")
    poscar = os.path.join(ROOT, "example", "test_POSCARs", "221_PPOSCAR_AlNi3")
    if not os.path.isfile(poscar):
        report("example POSCAR found", False, poscar)
        return
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        ceo2 = os.path.join(ROOT, "example", "visualized_basis", "CeO2_Fm-3m", "225_PPOSCAR_CeO2")
...
# ---------------------------------------------------------------- 25. crystod-phonon
def test_25_phonon_command() -> None:
    print("\n[25] crystod-phonon (sectioned command: 6 phonon modes)")
...
# ---------------------------------------------------------------- 26. crystod-group
def test_26_group_command() -> None:
    print("\n[26] crystod-group (sectioned command: 7 group-theory modes)")
...
# ---------------------------------------------------------------- 27. crystod main
def test_27_main_command() -> None:
    print("\n[27] crystod main command (SALC without mode flag)")
...
SECTIONS = {
    ...
    22: test_22_bz_command,
    23: test_23_md_command,
    24: test_24_mag_command,
    25: test_25_phonon_command,
    26: test_26_group_command,
    27: test_27_main_command,
}
